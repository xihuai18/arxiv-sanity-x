...
MinerU supports two backend modes and flexible device selection:

#### Backend Selection
```bash
# Pipeline backend (default, recommended) - Local GPU/CPU parsing
export ARXIV_SANITY_MINERU_BACKEND=pipeline

# VLM HTTP backend - Remote vLLM service
export ARXIV_SANITY_MINERU_BACKEND=vlm-http-client
```

#### Pipeline Backend Configuration (Recommended)

**1. GPU Mode (Default)**
```bash
# vars.py configuration
MINERU_BACKEND = "pipeline"
MINERU_DEVICE = "cuda"          # Use GPU
MINERU_MAX_WORKERS = 2          # Max 2 concurrent processes
MINERU_MAX_VRAM = 8             # Max 8GB VRAM per process
...
Features:
- ✅ GPU-accelerated parsing for faster processing
- ✅ Slot-based concurrency control to prevent VRAM exhaustion
- ✅ Supports `--vram` parameter to limit per-process VRAM
- ✅ Total VRAM usage = `MAX_WORKERS × MAX_VRAM` (e.g., 2×8GB=16GB)

**2. CPU Mode**
```bash
# vars.py configuration
MINERU_BACKEND = "pipeline"
MINERU_DEVICE = "cpu"           # Use CPU
# MAX_WORKERS and MAX_VRAM have no effect in CPU mode
```

Features:
- ✅ No GPU required, suitable for CPU-only servers
- ⚠️ Slower parsing performance
- ✅ No concurrency limits

**3. GPU Concurrency Control**

GPU slot mechanism (pipeline + cuda mode only):
- Uses file-based locks for cross-process synchronization
- Allows max `MINERU_MAX_WORKERS` concurrent GPU processes
- Limits each process to `MINERU_MAX_VRAM` GB VRAM
- Requests beyond limit wait for available slots (up to 10 minutes)
...
Check GPU slot usage:
```bash
ls -la data/mineru/.gpu_slots/
```

Force release all slots:
```bash
rm -rf data/mineru/.gpu_slots/
```

#### VLM HTTP Backend Configuration

Requires starting minerU's OpenAI-compatible service first:
```bash
# Start service on MINERU_PORT (default 52000)
export ARXIV_SANITY_MINERU_BACKEND=vlm-http-client
```

Features:
- ✅ Offload parsing to remote service
...
- `POST /api/get_paper_summary` - Get AI-generated paper summary (JSON: `{"pid": "paper_id", "model": "model_name", "force": false}`)
- `POST /api/clear_model_summary` - Clear summary cache for specific model (JSON: `{"pid": "paper_id", "model": "model_name"}`)
- `POST /api/clear_paper_cache` - Clear all caches for a paper (JSON: `{"pid": "paper_id"}`)

**Summary Page Clear Functions:**
- **Clear Current Summary**: Removes only the summary file generated by the currently selected model, preserving summaries from other models and all other caches (HTML, MinerU, etc.). Use this to regenerate a single model's summary.
- **Clear All**: Removes all related caches for the paper, including:
  - All model summary files (`data/summary/<pid>/*.md`)
  - HTML/Markdown cache (`data/html_md/<pid>/`)
  - MinerU cache (`data/mineru/<pid>/`)
  - All related metadata files

  Use this for a complete cache reset and full regeneration from scratch.
