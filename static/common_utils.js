...
(function (global) {
    ...
    function checkSummaryFallback(meta, requestedModel) {
        const resolvedModel = String((meta && meta.resolved_model) || '').trim();
        const displayModel = String((meta && meta.llm_model) || resolvedModel || '').trim();
        const actualModel = resolvedModel || displayModel;
        const requested = String(requestedModel || '').trim();
        const shouldShowNotice = displayModel && requested && displayModel !== requested;
        ...
        if (actualModel && requested && actualModel !== requested) {
            return {
                ...
                notice: shouldShowNotice
                    ? `Note: Fallback occurred. Summary generated by "${displayModel}" instead of "${requested}".`
                    : '',
            };
        }
