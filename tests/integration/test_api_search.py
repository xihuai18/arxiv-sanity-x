...
class TestTagSearchApi:
    ...
    def test_tag_search_missing_tag_returns_400(self, client):
        """Test that missing tag_name returns 400."""
        resp = client.post("/api/tag_search", json={})
        assert resp.status_code == 400

    def test_tag_search_with_tag_returns_success(self, client):
        """Test that valid tag search returns success structure."""
        resp = client.post(
            "/api/tag_search",
            json={"tag_name": "test_tag", "user": "testuser", "time_delta": 365, "limit": 5},
        )
...
class TestTagsSearchApi:
    ...
    def test_tags_search_missing_tags_returns_400(self, client):
        """Test that missing tags returns 400."""
        resp = client.post("/api/tags_search", json={})
        assert resp.status_code == 400

    def test_tags_search_with_tags_returns_success(self, client):
        """Test that valid tags search returns success structure."""
        resp = client.post(
            "/api/tags_search",
            json={"tags": ["test_tag"], "user": "testuser", "time_delta": 365, "limit": 5},
        )
        # May return 200 with empty results or 400 if tags don't exist
        assert resp.status_code in [200, 400]

    def test_tags_search_empty_tags_list(self, client):
        """Test tags search with empty tags list."""
        resp = client.post(
            "/api/tags_search",
            json={"tags": [], "user": "testuser"},
        )
        assert resp.status_code in [200, 400]
