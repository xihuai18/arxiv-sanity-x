...
# e.g. `python tools/compute.py` would otherwise miss sibling packages like `aslite/`.
...
from aslite.db import FEATURES_FILE, FEATURES_FILE_NEW, load_features, save_features
...
def _default_num_threads() -> int:
    # Keep behavior consistent with send_emails.py thread knobs.
    try:
    ...
    try:
        ...
        cap = 192
    base = cpu_count() or 2
    return min(base, cap) if requested <= 0 else min(requested, cap)


def _configure_num_threads(n: int) -> None:
...
def main(argv: list[str] | None = None) -> int:
    global USE_INTEL_EXT
    argv = sys.argv[1:] if argv is None else list(argv)

    # Configure numeric thread pools early.
    num_threads = _default_num_threads()
    _configure_num_threads(num_threads)
    ...
    print("Loading papers database...", flush=True)
    papers_db_local = PaperRepository.open_readonly()
    total_papers = len(papers_db_local)
    print(f"Database opened, {total_papers} papers found.", flush=True)

    # Load old features for comparison
    print("Loading existing features for comparison...", flush=True)
    old_pids = set()
    try:
        old_features = load_features()
        if old_features and "pids" in old_features:
            old_pids = set(old_features["pids"])
            print(f"Loaded {len(old_pids)} existing paper IDs.", flush=True)
    except Exception:
        print("No existing features found.", flush=True)

    # Count papers by category (only count arXiv categories)
    from collections import Counter

    print("Counting papers by category...", flush=True)
    ...
    for pid, p in tqdm(papers_db_local.items(), desc="Scanning papers", total=total_papers, ncols=100, file=sys.stderr):
        ...
        for tag in p.get("tags", []):
            term = tag.get("term", "")
            # Only count arXiv categories, skip ACM/MSC classification codes
            if term and (term.startswith(ARXIV_PREFIXES) or "." not in term and term.islower()):
                category_counts[term] += 1
                if is_new:
                    new_category_counts[term] += 1
